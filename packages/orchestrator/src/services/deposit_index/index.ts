...
class DepositIndexService {
  ...
  async removeDeposit({ receiver, bitcoinAddress, txid, vout }: DeleteParams) {
    ...
    this.logger.info(`Remove deposit with receiver ${receiver}, bitcoin address ${bitcoinAddress}, txid: ${txid}, vout: ${vout}`);
  }

  async getDepositsByReceiver(receiver: string, currentBtcHeight: number): Promise<DepositInfo[]> {
    const depositsInfo = [];
    const deposits: DepositIndexInterface[] = await this.db.select(TableName.DepositIndex, {
      where: { receiver }
    });
    for (const depositItem of deposits) {
      ...
      depositsInfo.push({
        deposit,
        confirmations: deposit.height ? currentBtcHeight - deposit.height + 1 : 0
      });
    }
    return depositsInfo;
  }

  async get({ receiver, bitcoinAddress, txid, vout }: GetParams): Promise<Deposit> {
    const data: DepositIndexInterface[] = await this.db.select(TableName.DepositIndex, {
      where: { receiver, bitcoinAddress, txid, vout }
    });
    if (data.length === 0) {
      return null;
