...
import { ITERATION_DELAY, RELAY_DEPOSIT_BLOCKS_SIZE, RELAY_HEADER_BATCH_SIZE, RETRY_DELAY } from '../../constants';
...
import WatchedScriptsService, { WatchedScriptsInterface } from '../watched_scripts';
...
class RelayerService implements RelayerInterface {
  ...
  constructor(btcClient: RPCClient, cwBitcoinClient: CwBitcoinClient, db: DuckDbNode) {
    this.btcClient = btcClient;
    this.cwBitcoinClient = cwBitcoinClient;
    this.watchedScriptClient = new WatchedScriptsService(db, this.cwBitcoinClient);
  ...
  async relay() {
    await Promise.all([this.relayHeader(), this.relayDeposit(), this.relayRecoveryDeposits(), this.relayCheckpoints(), this.relayCheckpointConf()]);
  }
  ...
  async relayHeader() {
    ...
    while (true) {
      try {
        let [fullNodeHash, sideChainHash] = await Promise.all([this.btcClient.getbestblockhash(), this.cwBitcoinClient.sidechainBlockHash()]);
  ...
  async relayHeaderBatch(fullNodeHash: string, sideChainHash: string) {
    ...
    let currentSidechainBlockHash = await this.cwBitcoinClient.sidechainBlockHash();
    ...
    try {
      ...
      for (const tx of detailMempoolTxs) {
        ...
        for (let vout = 0; vout < outputs.length; vout++) {
          ...
          setNestedMap(this.depositIndex, [toReceiverAddr(convertSdkDestToWasmDest(script.dest)), address, getTxidKey(txid, vout)], {
            txid,
            vout,
            height: undefined,
            amount: output.value
      ...
      for (const block of blocks) {
        ...
        for (const tx of txs) {
          try {
            ...
            console.log(`[MAYBE_RELAY_DEPOSIT] ${err?.message} at tx ${tx.txid}`);
          }
        }
      }
    } catch (err) {
  ...
  async maybeRelayDeposit(tx: BitcoinTransaction, blockHeight: number, blockHash: string) {
    ...
    for (let i = 0; i < outputs.length; i++) {
      ...
      if (isExistOutpoint === true) {
        setNestedMap(this.depositIndex, [toReceiverAddr(convertSdkDestToWasmDest(script.dest)), address, getTxidKey(txid, i)], undefined);
        continue;
      }

      setNestedMap(this.depositIndex, [toReceiverAddr(convertSdkDestToWasmDest(script.dest)), address, getTxidKey(txid, i)], {
        txid,
        vout: i,
        height: blockHeight,
        amount: output.value
  ...
  async relayCheckpointConf() {
    ...
    while (true) {
      try {
        let [confirmedIndex, unconfirmedIndex, lastCompletedIndex] = await Promise.all([this.cwBitcoinClient.confirmedIndex(), this.cwBitcoinClient.unhandledConfirmedIndex(), this.cwBitcoinClient.completedIndex()]);
        ...
        if (maybeConf !== null) {
          ...
          if (height > btcHeight - minConfs) {
            console.log(`Waiting for more confirmations to relay checkpoint confirm with tx ${txid} ...`);
            continue;
          }
          ...
          await wrappedExecuteTransaction(async () => {
            ...
            console.log(`Relayed checkpoint confirmation with tx ${txid} at tx ${tx.transactionHash}`);
          });
          relayed[txid] = true;
        }
      } catch (err) {
  ...
  async getPendingDeposits(receiver: string): Promise<DepositInfo[]> {
    const currentSidechainBlockHash = await this.cwBitcoinClient.sidechainBlockHash();
    ...
    const deposits = Array.from(addressMap.values()).flatMap((addressMapValue) =>
      Array.from(addressMapValue.values())
        .filter((deposit) => deposit) // Filter out falsy values
        .map((deposit) => {
          const confirmations = deposit.height ? currentBtcHeight - deposit.height + 1 : 0;
          return {
            deposit,
            confirmations
          };
