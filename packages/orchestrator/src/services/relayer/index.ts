...
import { Dest, WrappedHeader } from '@oraichain/bitcoin-bridge-contracts-sdk/build/CwBitcoin.types';
...
import { ITERATION_DELAY, RELAY_DEPOSIT_BLOCKS_SIZE, RELAY_HEADER_BATCH_SIZE, SCAN_BLOCKS_CHUNK_SIZE, SCAN_MEMPOOL_CHUNK_SIZE, SUBMIT_RELAY_CHECKPOINT_INTERVAL_DELAY, SUBMIT_RELAY_RECOVERY_TX_INTERVAL_DELAY } from '../../constants';
...
export default class RelayerService implements RelayerInterface {
  ...
  constructor(protected btcClient: RPCClient, protected lightClientBitcoinClient: LightClientBitcoinClient, protected appBitcoinClient: AppBitcoinClient, protected db: DuckDbNode, protected network?: BitcoinNetwork) {
    this.btcClient = btcClient;
    this.lightClientBitcoinClient = lightClientBitcoinClient;
    this.appBitcoinClient = appBitcoinClient;
    this.watchedScriptClient = new WatchedScriptsService(db, this.appBitcoinClient);
  ...
  async trackMemoryLeak() {
    while (true) {
      ...
      this.logger.info('=============================================');
      this.logger.info(`Memory heap total that GC predicts to allocate: ${currentHeapTotal} MB`);
      this.logger.info(`Memory heap total that GC actually used: ${currentHeapUsed} MB`);
      if (currentHeapUsed > 1200) {
        this.logger.error(`Heap is very high now, at ${currentHeapUsed} MB. Consider to check the process!`);
      }
      this.logger.info('=============================================');
  ...
  async relayHeader(lastHash: string | null) {
    ...
    try {
      let [fullNodeHash, sideChainHash] = await Promise.all([this.btcClient.getbestblockhash(), this.lightClientBitcoinClient.sidechainBlockHash()]);
      ...
      if (lastHash != fullNodeHash) {
        ...
        const lastHashHeader: BlockHeader = await this.blockHeaderService.getBlockHeader(lastHash);
        this.logger.info(`Sidechain header state is up-to-date:\n\thash=${lastHashHeader.hash}\n\theight=${lastHashHeader.height}`);
      }
    } catch (err) {
  ...
  async relayHeaderBatch(fullNodeHash: string, sideChainHash: string) {
    let [fullNodeInfo, sideChainInfo]: [BlockHeader, BlockHeader] = await Promise.all([this.blockHeaderService.getBlockHeader(fullNodeHash), this.blockHeaderService.getBlockHeader(sideChainHash)]);
    ...
    if (wrappedHeaders.length > 0) {
      ...
      let currentSidechainBlockHash = await this.lightClientBitcoinClient.sidechainBlockHash();
  ...
  async getHeaderBatch(blockHash: string): Promise<WrappedHeader[]> {
    let cursorHeader: VerbosedBlockHeader = await this.blockHeaderService.getBlockHeader(blockHash);
    let wrappedHeaders = [];
  ...
  async commonAncestor(leftHash: string, rightHash: string): Promise<BlockHeader> {
    let [leftHeader, rightHeader]: [BlockHeader, BlockHeader] = await Promise.all([this.blockHeaderService.getBlockHeader(leftHash), this.blockHeaderService.getBlockHeader(rightHash)]);
    while (leftHeader.hash !== rightHeader.hash) {
      if (leftHeader.height > rightHeader.height && rightHeader.confirmations - 1 === leftHeader.height - rightHeader.height) {
        return rightHeader;
      } else if (rightHeader.height > leftHeader.height && leftHeader.confirmations - 1 === rightHeader.height - leftHeader.height) {
        return leftHeader;
  ...
  async relayDeposit(prevTip: string | null) {
    ...
    try {
      ...
      if (prevTip === tip) {
        throw new Error('Current tip block is scanned for relaying. Waiting for next header...');
      }
      prevTip = prevTip || tip;
      let startHeight = (await this.commonAncestor(tip, prevTip)).height;

      let endHeader: BlockHeader = await this.blockHeaderService.getBlockHeader(tip);
      let endHeight = endHeader.height;
      let numBlocks = Math.max(endHeight - startHeight, RELAY_DEPOSIT_BLOCKS_SIZE);
      this.logger.info(`Scanning ${numBlocks} blocks for deposit transactions...`);
      ...
      while (i < mempoolTxs.length) {
        ...
        for (const tx of detailMempoolTxs) {
          ...
          for (let vout = 0; vout < outputs.length; vout++) {
            ...
            const script = await this.watchedScriptClient.getScript(output.scriptPubKey.hex);
  ...
  async scanDeposits(numBlocks: number) {
    try {
      ...
      let allBlockhashes = (await this.btcClient.batch(allHeightQuerier)).map((item) => item.result);
      let i = 0;
      while (i < allBlockhashes.length) {
        const blockhashChunk = mapSlice(allBlockhashes, i, (i += SCAN_BLOCKS_CHUNK_SIZE), (blockhash: string) => ({
          method: 'getblock',
          params: [blockhash, 2]
        }));

        let allDetailBlocks = (await this.btcClient.batch(blockhashChunk)).map((item) => item.result);
        for (const block of allDetailBlocks) {
          ...
          for (const tx of txs) {
            try {
              ...
              this.logger.error(`[MAYBE_RELAY_DEPOSIT] Error at tx ${tx.txid}:`, err);
            }
          }
        }
      }
    ...
    let results = await Promise.all(
      txs.map(async (tx) => {
        ...
        for (const output of outputs) {
          if (output.scriptPubKey.type == ScriptPubkeyType.WitnessScriptHash) {
            let script = await this.watchedScriptClient.getScript(output.scriptPubKey.hex);
            if (script) {
              return true;
            }
          }
  ...
  async maybeRelayDeposit(tx: BitcoinTransaction, blockHeight: number, blockHash: string) {
    const txid = tx.txid;
    const rawTx = tx.hex;
    const outputs = tx.vout;
    for (let i = 0; i < outputs.length; i++) {
      ...
      if (output.scriptPubKey.type != ScriptPubkeyType.WitnessScriptHash) continue;
      ...
      await wrappedExecuteTransaction(async () => {
        const tx = await this.appBitcoinClient.relayDeposit({
          ...
          btcTx: Buffer.from(toBinaryTransaction(decodeRawTx(rawTx))).toString('base64'),
          btcProof: Buffer.from(toBinaryPartialMerkleTree(fromBinaryMerkleBlock(Buffer.from(txProof, 'hex')).txn)).toString('base64'),
          btcVout: i,
          dest: script.dest,
          sigsetIndex: Number(script.sigsetIndex)
        });
        this.logger.info(`Relayed deposit tx ${txid} at tx ${tx.transactionHash}`);
      }, this.logger);
    }
  }
  ...
  async relayRecoveryDeposits() {
    ...
    try {
      ...
      for (const recoveryTx of txs) {
        const existed = await this.relayedSetService.exist(`relay-recovery-deposits-${recoveryTx}`);
        if (existed) continue;
        const tx = await this.btcClient.sendrawtransaction({
          hexstring: Buffer.from(recoveryTx, 'base64').toString('hex')
        });
        await this.relayedSetService.insert(`relay-recovery-deposits-${recoveryTx}`);
  ...
  async relayCheckpoints() {
    ...
    try {
      ...
      for (const checkpoint of checkpoints) {
        const exist = await this.relayedSetService.exist(`relay-checkpoint-${checkpoint}`);
        if (exist) continue;
        try {
          const tx = await this.btcClient.sendrawtransaction({
  ...
  async relayCheckpointConf() {
    ...
    try {
      let [confirmedIndex, unconfirmedIndex, lastCompletedIndex] = await Promise.all([this.appBitcoinClient.confirmedIndex(), this.appBitcoinClient.unhandledConfirmedIndex(), this.appBitcoinClient.completedIndex()]);

      if (unconfirmedIndex === null) {
        throw new Error('No unconfirmed checkpoint index. Waiting for next scan...');
      }

      let unconfIndex = Math.max(unconfirmedIndex, lastCompletedIndex - 5);

      if (confirmedIndex !== null && confirmedIndex == unconfIndex) {
        throw new Error('Unconfirmed checkpoint index is confirmed. Waiting for next scan...');
      }
      ...
      let [tx, btcHeight, minConfs] = await Promise.all([
        ...
        (async () => {
          const blockHash = await this.lightClientBitcoinClient.sidechainBlockHash();
          const blockHeader = await this.blockHeaderService.getBlockHeader(blockHash);
      ...
      const exist = await this.relayedSetService.exist(`relay-checkpoint-conf-${txid}`);
      if (exist) {
        throw new Error(`Checkpoint confirm with tx ${txid} is already relayed`);
      }
      ...
      if (maybeConf !== null) {
        ...
        if (height > btcHeight - minConfs) {
          this.logger.info(`Waiting for more confirmations to relay checkpoint confirm with tx ${txid} ...`);
          return;
        }
        ...
        await wrappedExecuteTransaction(async () => {
          ...
          this.logger.info(`Relayed checkpoint confirmation with tx ${txid} at tx ${tx.transactionHash}`);
        }, this.logger);
        await this.relayedSetService.insert(`relay-checkpoint-conf-${txid}`);
      }
    } catch (err) {
      if (!err?.message.includes('Waiting for next scan...') && !err?.message.includes('No completed checkpoints yet')) {
        this.logger.error(`[RELAY_CHECKPOINT_CONF] Error:`, err);
      }
    }
  }

  async scanForTxid(txid: string, numBlocks: number, scanBlocks: number): Promise<[number, string] | null> {
    let tip = await this.lightClientBitcoinClient.sidechainBlockHash();
  ...
  async getPendingDeposits(receiver: string): Promise<DepositInfo[]> {
    const currentSidechainBlockHash = await this.lightClientBitcoinClient.sidechainBlockHash();
    const blockHeader: BlockHeader = await this.blockHeaderService.getBlockHeader(currentSidechainBlockHash);
    const currentBtcHeight = blockHeader.height;
    return this.depositIndexService.getDepositsByReceiver(receiver, currentBtcHeight);
  }

  // for testing purpose only
  async generateDepositAddress(checkpointIndex: number, dest: Dest, network: btc.networks.Network) {
  ...
  async submitDepositAddress(depositAddr: string, checkpointIndex: number, dest: Dest): Promise<void> {
    const checkpoint = await (checkpointIndex
      ? this.appBitcoinClient.checkpointByIndex({
          index: checkpointIndex
        })
    ...
    if (address !== depositAddr) {
      throw new Error(`Generated address ${address} does not match the expected address ${depositAddr}`);
    }

    let currentTime = Math.floor(Date.now() / 1000);
    if (currentTime + env.deposit.depositBuffer >= sigset.create_time + bitcoinConfig.max_deposit_age) {
      throw new Error('Sigset no longer accepting deposits. Unable to generate deposit address');
    }
