...
export default class RelayerService implements RelayerInterface {
  ...
  async relayHeader(lastHash: string | null) {
    ...
    try {
      ...
      const sideChainHash = await this.lightClientBitcoinClient.sidechainBlockHash();
  ...
  async relayHeaderBatch(fullNodeHash: string, sideChainHash: string) {
    const fullNodeInfo = await this.blockHeaderService.getBlockHeader(fullNodeHash);
    const sideChainInfo = await this.blockHeaderService.getBlockHeader(sideChainHash);
  ...
  async relayCheckpointConf() {
    ...
    try {
      ...
      let unconfirmedIndex = await this.appBitcoinClient.unhandledConfirmedIndex();
