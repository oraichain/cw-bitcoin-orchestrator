#!/bin/bash
set -u
MNEMONIC_VALIDATOR_1="rough minor raise pelican rate dog camera fold scissors asthma tuition gaze silver mom borrow bicycle produce witness forest blush law arctic name issue"
MNEMONIC_VALIDATOR_2="ski office crowd wine fabric digital cricket toward ripple tattoo live amount"
HIDE_LOGS="/dev/null"
...
echo $MNEMONIC_VALIDATOR_1 | oraid keys add validator1 --recover --keyring-backend=test --home=$HOME/oraid/validator1 > $HIDE_LOGS
echo $MNEMONIC_VALIDATOR_2 | oraid keys add validator2 --recover --keyring-backend=test --home=$HOME/oraid/validator2 > $HIDE_LOGS
...
echo "Waiting 3 seconds to send funds to validators 2..."
sleep 3
